...
public interface HelperCar  extends HelperBase{
    ...
    //variant po koordinatam
    default void typeSerialNumber2(String text){
        Rectangle rect = wd.findElement(By.id("serialNumber")).getRect();
        int x = rect.getX() + rect.getWidth() * 7 / 8;
        int y = rect.getY() + rect.getHeight() / 2;
        Actions actions =new Actions(wd);
        actions.moveByOffset(x, y).click().sendKeys(text).perform(); //variant 1
//        wd.switchTo().activeElement().sendKeys(text);  variant 2 zapolneniya
